...
function search(){
    for(var item in watches[selectedbrand]){
        if(selectedproduct == watches[selectedbrand][item].name){        
            var card = document.getElementById("card-parent") 
            // card.classList.remove("d-none")
            card.innerHTML = `
                  <div class="card d-block d-sm-block d-md-none d-lg-none mt-5"  >
                    <img class="mt-4" height="300px" width="100%" src="${watches[selectedbrand][item].img}" alt="">
                    <div style="width: 90%; margin: 0px auto;  padding-top: 10px; class="info " > 
                        <p class=" mt-4 mb-2 values text-start"><b>Name: </b>${watches[selectedbrand][item].name}</p>
                        <p class="my-2 text-start values"><b>Price: </b> ${watches[selectedbrand][item].price}</p>
                        <p class="my-2 text-start values"><b>Full Name: </b> ${watches[selectedbrand][item].fullName}</p>
                        <p class="my-2 text-start values"><b>Category: </b> ${watches[selectedbrand][item].category}</p>
                        <p class="my-2 text-start values"><b>Caption: </b> ${watches[selectedbrand][item].caption}</p>
                        <p class="my-2 text-start values"><b>Tags: </b> ${watches[selectedbrand][item].Tags}</p>
                        <p class="mt-2 mb-4 text-start values"><b>Color: </b> ${watches[selectedbrand][item].color}</p>
                    </div>
                  </div>
                  <div class="card-md d-none d-sm-none d-md-flex  " style="width:100%;" >
                    <div class="div-img">
                      <img style="display:flex; justify-content:center; align-items:center; margin: 0px;" height="300px"  width="300px" src="${watches[selectedbrand][item].img}" alt="">
                    </div>
                  <div class="div text-center " style="margin: 0px auto; width:80% ;">
                    <p class="text-start values mt-3"><b>Name: </b>${watches[selectedbrand][item].name}</p>
                    <p class="text-start values"><b>Price: </b> ${watches[selectedbrand][item].price}</p>
                    <p class="text-start values"><b>Full Name: </b>${watches[selectedbrand][item].fullName}</p>
                    <p class="text-start values"><b>Category: </b>${watches[selectedbrand][item].category}</p>
                    <p class="text-start values"><b>Caption: </b>${watches[selectedbrand][item].caption}</p>
                    <p class="text-start values"><b>Tags: </b>${watches[selectedbrand][item].Tags}</p>
                    <p class="text-start values"><b>Color: </b>${watches[selectedbrand][item].color}</p>
                </div>
